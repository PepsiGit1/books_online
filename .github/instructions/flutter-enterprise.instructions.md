...
- CI policy mode: advisory-only
- Stack is generated from blueprint user selections (not hardcoded defaults).

### Selected Stack Snapshot

- State management: bloc/cubit (generated by user selection; event/state separation required)
- Dependency injection: Use constructor injection first; optionally use get_it/injectable for scale
...
- Offline storage: Hive not selected.
...
CI mode for this generated policy: advisory-only
